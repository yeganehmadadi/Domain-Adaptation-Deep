...
class NewModel(nn.Module):
    ...
    def __init__(self, num_classes=65):
        ...
        self.sharedNet1 = resnet50(True)
        self.sharedNet2 = resnet50(True)
    ...
    def forward(self, data_src, data_tgt = 0, label_src = 0, mark = 1):
        global data_tgt_son1
        lr_loss = 0
        if self.training == True:
            if mark == 1:
                data_src = self.sharedNet1(data_src)
                data_tgt = self.sharedNet2(data_tgt)
                ...
                lr_loss += LowRank.LowRank(data_src.cpu().detach(), data_tgt_son1.cpu().detach())              
                ...
                #my reweight sampling
                class_weight = 1
                cls_loss_w = 0
                if lr_loss > 0.01:
                    class_weight = torch.mean(pred_src)
                    class_weight = (class_weight / torch.mean(class_weight)).cuda().view(-1)
                    cls_loss_w = torch.mul(cls_loss, class_weight)

                #return cls_loss, mmd_loss, l1_loss
                return cls_loss, lr_loss, l1_loss, cls_loss_w
            ...
            if mark == 2:
                ...
                lr_loss += LowRank.LowRank(data_src.cpu().detach(), data_tgt_son2.cpu().detach())
                ...
                return cls_loss, mmd_loss, l1_loss
                #return cls_loss, mmd_loss, l1_loss, cls_loss_w

        else:
            data = self.sharedNet1(data_src)
